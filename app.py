...
app = Flask(__name__)
app.secret_key = "socialpay_secret_key_2024_xk9z"
...
BANK_FILE        = "data/bank_details.json"
...
AUDIT_FILE       = "data/logs/audit.json"
...
def login():
    if request.method == "POST":
        ...
        # Store temp login data and send OTP
        otp = generate_otp()
        save_otp(email, otp, "login")
        session["pending_login"] = {"uid": uid, "email": email}
        lang = udata.get("lang", "en")
        session["lang"] = lang

        ok = send_email(email, f"[{APP_NAME}] Your OTP Code",
                        otp_email_html(otp, udata.get("name", "User"), lang))

        if not ok:
            # fallback: auto-login without OTP for testing
            session["user_id"] = uid
...
def register():
    ...
    if request.method == "POST":
        ...
        session["pending_register"] = {"uid": uid, "email": email,
                                        "name": name, "user_data": user_data,
                                        "ref_code": ref_code}

        ok = send_email(email, f"[{APP_NAME}] Verify Your Email",
                        otp_email_html(otp, name, lang))
        if not ok:
            # Fallback: auto-register without OTP
            users[uid] = user_data
            users[uid]["verified"] = True
            save(USERS_FILE, users)
            get_wallet(uid)
...
def submit_task():
    ...
    subs[sid] = {"id": sid, "user_id": uid, "task_id": task_id,
                 "proof": proof[:1000], "status": "pending",
...
if __name__ == "__main__":
    ...
    print(f"  📧 OTP Email: {EMAIL_USER}")
    print(f"  🌍 Languages: English, Arabic, Hausa")
    print("=" * 55)
